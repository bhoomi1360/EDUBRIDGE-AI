...
export default function AdminPanel({ activeSection, setActiveSection }) {
  const {
    students,
    addStudent,
    deleteStudent,
  ...
  // Map sidebar activeSection id to sub-tab name
  const getSubTabFromSection = (section) => {
    switch (section) {
      case 'admin_students': return 'students';
      case 'admin_jobs': return 'jobs';
      case 'admin_invites': return 'invites';
      case 'admin_curriculum': return 'curriculum';
      case 'admin_challenges': return 'challenges';
      default: return 'students';
    }
  };

  const [activeSubTab, setActiveSubTab] = useState(() => getSubTabFromSection(activeSection));
  const [searchTerm, setSearchTerm] = useState('');
  const [notification, setNotification] = useState(null);

  // Sync when activeSection from sidebar changes
  useEffect(() => {
    if (activeSection) {
      setActiveSubTab(getSubTabFromSection(activeSection));
    }
  }, [activeSection]);
  ...
  const handleSubTabChange = (tabName) => {
    ...
    if (setActiveSection) {
      ...
      if (sectionMap[tabName]) {
        setActiveSection(sectionMap[tabName]);
      }
    }
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div className="glass-card flex-between" style={{ gap: '20px', flexWrap: 'wrap', background: 'linear-gradient(135deg, rgba(168, 85, 247, 0.15) 0%, rgba(99, 102, 241, 0.15) 100%)' }}>
        <div>
          <span className="badge badge-purple" style={{ marginBottom: '8px' }}>
            <ShieldCheck size={12} /> Master Control & Admin Management
          </span>
          <h2 style={{ fontSize: '1.6rem', fontWeight: 800 }}>
            Dedicated <span className="gradient-text">Admin Panel</span>
          </h2>
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', marginTop: '4px' }}>
            Full control center to add new items, modify existing data, or delete records across all platform modules.
          </p>
        </div>
      ...
      <div className="grid-4" style={{ gap: '14px' }}>
        ...
        <div 
          onClick={() => handleSubTabChange('invites')}
          className="glass-card" 
          style={{ 
            padding: '16px', 
            cursor: 'pointer', 
